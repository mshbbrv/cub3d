...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/12 14:24:38 by thjonell          #+#    #+#             */
/*   Updated: 2021/03/12 14:26:36 by thjonell         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sprites_dist_calc(t_all_data *all)
{
	...
	while (i < all->sprite_data.num_sprites)
	{
		...
		all->sprite_data.dist[i] = ((all->map_data.pl_x -
				all->sprite_data.sprite[i].x) *
						(all->map_data.pl_x - all->sprite_data.sprite[i].x) +
						(all->map_data.pl_y - all->sprite_data.sprite[i].y) *
						(all->map_data.pl_y - all->sprite_data.sprite[i].y));
		i++;
	}
}
...
{
	all->sprite_data.inv_det = 1.0 / (all->rc.cam_plane_x *
			all->rc.pl_dir_y - all->rc.pl_dir_x * all->rc.cam_plane_y);
	all->sprite_data.trans_x = all->sprite_data.inv_det * (all->rc.pl_dir_y *
			all->sprite_data.x - all->rc.pl_dir_x * all->sprite_data.y);
	all->sprite_data.trans_y = all->sprite_data.inv_det *
			(-all->rc.cam_plane_y * all->sprite_data.x + all->rc.cam_plane_x
			* all->sprite_data.y);
...
void	render_sprite(t_all_data *all)
{
	...
	{
		all->sprite_data.x =
				all->sprite_data.sprite[all->sprite_data.order[i]].x
				- all->map_data.pl_x;
		all->sprite_data.y =
				all->sprite_data.sprite[all->sprite_data.order[i]].y
				- all->map_data.pl_y;
